...
// auth/db/storage methods from useEffect/event handlers (browser-only), so it
// is safe to lazily no-op on the server and initialize for real in the browser.
...
export const storage: FirebaseStorage = isBrowser
  ? getStorage(firebaseApp as FirebaseApp)
