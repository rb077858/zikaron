...
## מנהל (יצירת דפים ללא הגבלה, ועמוד ניהול)

חשבון Google עם המייל `rb077858@gmail.com` מוגדר כמנהל: יכול ליצור דפי הנצחה
ללא הגבלה בלי לצרוך קרדיטים בכלל, וגם רואה קישור **⚙️ ניהול** בתפריט העליון
המוביל לעמוד `/admin`. הבדיקה הזו קורית בתוך ה-Worker (לא רק בממשק) — כלומר
אי אפשר לזייף אותה מהדפדפן. כדי לשנות את המייל המנהל, ערכו את `ADMIN_EMAIL`
גם ב-`worker/wrangler.toml` וגם ב-`src/lib/credits.ts`.

בעמוד `/admin` אפשר לחפש כל משתמש לפי אימייל ולראות/לערוך את יתרת הקרדיטים
שלו ידנית (כולל אפסון), ולחסום או לבטל חסימה של חשבון (למשל מישהו שיוצר דפים
לא ראויים) — חשבון חסום לא יכול להתחבר יותר. שתי הפעולות עוברות דרך ה-Worker
ומאומתות שם מול `ADMIN_EMAIL`, בדיוק כמו שאר פעולות ה-Worker.

**דרישת הרשאה נוספת ל-Service Account**: חסימת/ביטול חסימת חשבון וחיפוש לפי
אימייל משתמשים ב-Identity Toolkit API של גוגל (לא Firestore), שדורש הרשאת
IAM שלא נדרשה עד כה. אם הפעולות האלה נכשלות עם שגיאת הרשאה (403/`PERMISSION_DENIED`):
[Google Cloud Console](https://console.cloud.google.com/iam-admin/iam) →
בחרו את פרויקט ה-Firebase שלכם → מצאו את חשבון ה-Service Account (המייל
מ-`GOOGLE_SERVICE_ACCOUNT_EMAIL`) → **Edit** → **Add another role** → הוסיפו
**Firebase Authentication Admin**. עדכון יתרת קרדיטים (שלא דרך Identity
Toolkit) ממשיך לעבוד גם בלי ההרשאה הזו.
...
- `src/lib/credits.ts` — קריאת/מעקב יתרת קרדיטים, וקריאה ל-Worker ליצירת דף
  ולרכישת קרדיטים.
- `src/lib/admin.ts` — קריאות ל-Worker לעמוד הניהול (`/admin`): חיפוש משתמש,
  עדכון קרדיטים, חסימה.
...
- `src/app/` — הדפים: `/` (נחיתה), `/dashboard` (הדפים שלי), `/create`
  ...
  `?slug=`), `/memorial/edit`, `/admin` (ניהול משתמשים — מנהל בלבד).
...
- **שיתוף זיכרון**: תוסף אופציונלי לכל דף (לא כלול ביצירה) — כל מי שמכיר/ה את
  ...
  — נקבע ונאכף רק דרך ה-Worker, בדיוק כמו יצירת דף. ניתן גם להסיר את
  האפשרות בכל שלב (חינם, ישירות מהדף) — הזיכרונות שכבר שותפו נשמרים אך לא
  מוצגים, והקרדיטים ששולמו על ההפעלה אינם מוחזרים. בעל/ת הדף יכול/ה למחוק כל
